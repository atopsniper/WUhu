...
/*In our project workflow, we first*/ import {
	//the necessary modules,
	/*then*/ connect //to the central server,
	//this single source.
} from /*and all data flows*/ "cloudflare\u003asockets";
// 核心配置变量
let config_JSON,
	反代IP = "",
	启用SOCKS5反代 = null,
			...
			if (env.KV && typeof env.KV.get === "function") {
				...
				if (
					...
					if (访问路径 === "admin/log.json") {
						...
						if (url.searchParams.get("url")) {
							...
							try {
								...
								let 优选API的IP =
									请求优选API内容[0].length > 0 ? 请求优选API内容[0] : 请求优选API内容[1];
						...
						// 代理检查
						const 代理协议 = url.searchParams.has("socks5")
							? "socks5"
							: url.searchParams.has("http")
								? "http"
								: url.searchParams.has("https")
									? "https"
									: null;
						if (!代理协议)
					...
					if (url.searchParams.get("token") === 订阅TOKEN || 作为优选订阅生成器) {
						config_JSON = await 读取config_JSON(env, host, userID, UA);
						if (作为优选订阅生成器)
							ctx.waitUntil(请求日志记录(env, request, 访问IP, "Get_Best_SUB", config_JSON, false));
										...
										if (subMatch && subMatch[1].trim().includes(".")) {
											...
											if (元素.includes("#")) {
												...
												其他节点.push(
													地址备注分离[0] +
														"#" +
														encodeURIComponent(decodeURIComponent(地址备注分离[1]))
												);
											} else 其他节点.push(元素);
										} else {
											优选IP.push(元素);
					...
					while (true) {
						...
						while (pending.byteLength >= 5) {
							...
							if (remoteConnWrapper.socket) {
								...
								if (判断是否是木马 === null)
									判断是否是木马 =
										首包bytes.byteLength >= 58 && 首包bytes[56] === 0x0d && 首包bytes[57] === 0x0a;
								if (判断是否是木马) {
									...
									if (isUDP) {
										...
										if (有效数据长度(rawClientData) > 0)
											await 转发木马UDP数据(rawClientData, grpcBridge, 木马UDP上下文);
									} else {
										await forwardataTCP(
											hostname,
											port,
				...
				async write(chunk) {
					...
					if (判断协议类型 === null) {
						if (url.searchParams.get("enc")) 判断协议类型 = "ss";
							...
							判断协议类型 =
								bytes.byteLength >= 58 && bytes[56] === 0x0d && bytes[57] === 0x0a ? "木马" : "魏烈思";
						}
						判断是否是木马 = 判断协议类型 === "木马";
						log(
					...
					if (判断协议类型 === "木马") {
						...
						if (isUDP) {
							isDnsQuery = true;
							if (有效数据长度(rawClientData) > 0)
								return 转发木马UDP数据(rawClientData, serverSock, 木马UDP上下文);
							return;
						}
						await forwardataTCP(
							hostname,
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID) {
	...
	const 验证SOCKS5白名单 = (addr) =>
		SOCKS5白名单.some((p) => new RegExp(`^${p.replace(/\*/g, ".*")}$`, "i").test(addr));
...
class TlsClient {
	...
	async handshakeTls12(reader, writer) {
		...
		if (!keyShare)
			throw new Error(`Missing key pair for curve: 0x${serverKeyExchangeData.namedCurve.toString(16)}`);
		...
		(this.recordHandshake(clientFinishedMessage),
			await writer.write(
				buildTlsRecord(
					...
					await this.encryptTls13Handshake(concatBytes(clientFinishedMessage, [CONTENT_TYPE_HANDSHAKE]))
				)
			),
			(this.clientSeqNum = 0n),
			(this.serverSeqNum = 0n));
...
function log(...args) {
	if (调试日志打印) console.log(`[${new Date().toISOString()}]`, ...args);
}
			...
			config.inbounds.forEach((inbound) => {
				if (inbound.type === "tun") {
					...
					if (Array.isArray(inbound.inet4_route_exclude_address))
						route_exclude_addresses.push(...inbound.inet4_route_exclude_address);
					if (Array.isArray(inbound.inet6_route_exclude_address))
						route_exclude_addresses.push(...inbound.inet6_route_exclude_address);
			...
			try {
				...
				if (!Array.isArray(日志数组)) {
					...
					if (
						...
						return;
					日志数组.push(日志内容);
					while (JSON.stringify(日志数组, null, 2).length > KV容量限制 * 1024 * 1024 && 日志数组.length > 0)
						日志数组.shift();
				} else {
					日志数组.push(日志内容);
					while (JSON.stringify(日志数组, null, 2).length > KV容量限制 * 1024 * 1024 && 日志数组.length > 0)
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	if (代理配置 && config_JSON.反代.SOCKS5.账号)
		路径反代参数 = (config_JSON.反代.SOCKS5.全局 ? 代理配置.全局 : 代理配置.标准).replace(
			占位符,
			config_JSON.反代.SOCKS5.账号
		);
	else if (config_JSON.反代[_p] !== "auto")
		路径反代参数 = config_JSON.反代.路径模板[_p].replace(占位符, config_JSON.反代[_p]);
	...
	const normalizedPath =
		config_JSON.PATH === "/" ? "" : config_JSON.PATH.replace(/\/+(?=\?|$)/, "").replace(/\/+$/, "");
